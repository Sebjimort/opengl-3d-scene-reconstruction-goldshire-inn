...
private:
    ...
    void setupMesh()
    {
        // --- PARCHE DE SEGURIDAD PARA PREVENIR CRASH EN LA GPU ---
        // Si el sub-modelo no tiene huesos, rellenamos el arreglo con ceros para que iguale a los vértices
        if (bones_id_weights_for_each_vertex.size() != vertices.size())
        {
            bones_id_weights_for_each_vertex.resize(vertices.size());
        }
        // ---------------------------------------------------------
